...
void CCarrier::LateUpdate()
{
	...
	if (STATE::DEAD == m_eCurState)
	{
		...
		{
			CSoundMgr::GetInstance()->MyPlaySound(L"PCaDth00.wav", CSoundMgr::UI);
			m_bIsDead = true;
		}
	}
}

void CCarrier::Render()
{
...
void CCarrier::StateCurFrameCheck()
{
	switch (m_eDir)
	{
	case CUnit::LEFT:
