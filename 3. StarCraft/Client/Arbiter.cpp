...
void CArbiter::LateUpdate()
{
	...
	if (STATE::DEAD == m_eCurState)
	{
		...
		{
			m_bIsDead = true;
			CSoundMgr::GetInstance()->MyPlaySound(L"pabDth00.wav", CSoundMgr::UI);
		}
	}
}

void CArbiter::Render()
{
	CDeviceMgr::GetInstance()->GetSprite()->SetTransform(&m_tInfo.matWorld);

	const TEX_INFO* test = CTextureMgr::GetInstance()->FindTexture(L"Unit_Arbiter", m_wsUnitStateKey[m_eCurState], m_tFrame.iFrameStart);

	CDeviceMgr::GetInstance()->GetSprite()->Draw(
		CTextureMgr::GetInstance()->FindTexture(L"Unit_Arbiter", m_wsUnitStateKey[m_eCurState], m_tFrame.iFrameStart)->pTexture,
...
void CArbiter::StateCurFrameCheck()
{
	switch (m_eDir)
	{
	case CUnit::LEFT:
