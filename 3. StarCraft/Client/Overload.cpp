...
void COverload::LateUpdate()
{
	...
	if (STATE::DEAD == m_eCurState)
	{
		...
		{
			CSoundMgr::GetInstance()->MyPlaySound(L"OverloadDaad.wav", CSoundMgr::UI);
			m_bIsDead = true;
		}
	}
}

void COverload::Render()
{
...
void COverload::StateCurFrameCheck()
{
	switch (m_eDir)
	{
	case CUnit::LEFT:
		m_tInfo.vDir = { -1.0f, 0.0f, 0.0f };
		m_iStateCurFrame[STATE::WORK] = 8;
		m_iNextLook = 0;
		break;
	case CUnit::RIGHT:
		m_tInfo.vDir = { 1.0f, 0.0f, 0.0f };
	...
	case CUnit::UP:
		...
		m_iStateCurFrame[STATE::WORK] = 4;
		m_iNextLook = 2;
		break;
	case CUnit::DOWN:
		m_tInfo.vDir = { 0.0f, 1.0f, 0.0f };
		m_iStateCurFrame[STATE::WORK] = 12;
		m_iNextLook = 3;
		break;
	default:
		break;
...
void COverload::FrameCheck()
{
	...
	case STATE::WORK:
		...
		m_tFrame.iFrameEnd = m_iStateCurFrame[STATE::WORK] + 3;
		m_tFrame.iFrameScene = 0;
